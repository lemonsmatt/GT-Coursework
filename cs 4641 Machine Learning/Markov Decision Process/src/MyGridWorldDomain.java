...
public class MyGridWorldDomain implements DomainGenerator {
	...
	public static void main(String[] args) {
		...
		MyMap map = new MyMap(0.8, false);
		gwdg.setMap(map.getCoordinates());
		Domain d = gwdg.generateDomain();


		State s = getOneAgentNLocationState(d, map.getCols().length);
		setAgent(s, 0, 0);
		for (int i = 0; i <map.getCols().length; i++)
		{
			setLocation(s, i, map.getRows()[i]-1, map.getCols()[i]-1, 0);
		}
