...
public class MyGridWorld {
	...
	public MyGridWorld (MyMap map, double goalReward, double defaultReward, double discount){
		...
		int[] columns = map.getCols();
		this.gw = new GridWorldDomain(map.getCoordinates());
		gw.setMap(map.getCoordinates());
		gw.setProbSucceedTransitionDynamics(map.getSuccessRate());
		this.domain = gw.generateDomain();
		this.initialState = GridWorldDomain.getOneAgentNLocationState(domain, rows.length);
		GridWorldDomain.setAgent(initialState, 0, 0);

		for(int i = 0; i < rows.length; i++)
		{
			GridWorldDomain.setLocation(initialState, i, rows[i]-1, columns[i]-1);
